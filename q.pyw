...
def get_bg_color_by_wq_id(wq_id: str) -> str:
    return _BG_COLOR_MAP.get(wq_id, q1)

def get_selection_colors():
    return _SELECTION_THEMES.get(SELECTION_THEME, (QColor("#ff4500"), QColor("#ffffff")))

def apply_selection_palette(widget: QWidget):
    """统一的选区颜色：只走 QPalette，不走 QSS，避免多处重复/覆盖。"""
    sel_bg, sel_fg = get_selection_colors()
    pal = widget.palette()
    pal.setColor(QPalette.Highlight, sel_bg)
    pal.setColor(QPalette.HighlightedText, sel_fg)
    widget.setPalette(pal)

def build_editor_qss(bg_color: str) -> str:
    """编辑器样式：只管背景/滚动条/边框，不在这里写 selection-*。"""
    return f'''
    QPlainTextEdit {{
        background:{bg_color};
        color:{q2};
...
def apply_editor_theme(editor: QPlainTextEdit, bg_color: str):
    """给编辑器统一套样式 + 统一选区配色（关键：避免三处改色）。"""
    editor.setStyleSheet(build_editor_qss(bg_color))
    apply_selection_palette(editor)

def build_titlebar_qss(bg_color: str) -> str:
    return f"background:{bg_color}; border:none;"

def build_window_qss(bg_color: str) -> str:
    return f"background:{bg_color}; border:none;"
...
def _limit_queue_size():
    try:
        ...
        while total_size > _QUEUE_MAX_SIZE and files:
            ...
            oldest_path, oldest_size, _ = files.pop(0)
            try:
    ...
    for p in glob.glob(os.path.join(wq_dir, f"{prefix}_instance_*.lock")):
        try:
            ...
            if filename.startswith(f"{prefix}_instance_") and filename.endswith(".lock"):
                group_char = filename[len(f"{prefix}_instance_"):-5]
...
def _is_token_sep(ch: str) -> bool:
    if not ch:
        return True
    if ch.isspace():
        return True
    return ch in _TOKEN_SEP
...
class q19(QPlainTextEdit):
    ...
    def __init__(q20):
        ...
        q20._qbox_line = None
        q20._qbox_ms = 96
        q20._qbox_timer = QTimer(q20)
        q20._qbox_timer.setSingleShot(True)
        q20._qbox_timer.timeout.connect(q20._flush_qbox_highlight)
        ...
        # ✅ 关键：编辑器主题统一从一个函数来（不写 selection-*）
        apply_editor_theme(q20, q1)
    ...
    def bind_qbox(q20, line_edit: QLineEdit):
        q20._qbox_line = line_edit

    def _qbox_text(q20) -> str:
        try:
            if q20._qbox_line is None:
                return ""
            t = q20._qbox_line.text()
            t = _normalize_text(t)
    ...
    def _qbox_active(q20) -> bool:
        return bool(q20._qbox_text())

    def _schedule_qbox_highlight(q20):
        try:
            q20._qbox_timer.start(q20._qbox_ms)
        except Exception:
            q20.q96()

    def _flush_qbox_highlight(q20):
        try:
            q20._apply_match_highlight(q20._needle())
        except Exception:
            pass
    ...
    def _on_contents_change(q20, pos, removed, added):
        ...
        # ✅ 原逻辑会清掉匹配高亮；但 q框必须持续生效
        if q20._last_needle and (not q20._qbox_active()):
            try:
    ...
    def _save_hover_image(q20):
        ...
        path, _ = QFileDialog.getSaveFileName(q20, "另存为图片", start_path, filter_str)
        if not path:
            q20._update_hover()
            return
    ...
    def _needle(q20):
        ...
        if pos == 0:
            ch_r = line[0] if len(line) > 0 else ""
            if not ch_r or _is_token_sep(ch_r):
                return ""
            idx = 0
        else:
            ch_l = line[pos - 1] if pos - 1 < len(line) else ""
            if _is_token_sep(ch_l):
                return ""
            idx = pos - 1

        if idx < 0 or idx >= len(line):
            return ""
        if _is_token_sep(line[idx]):
            return ""

        a = idx
        while a > 0 and (not _is_token_sep(line[a - 1])):
            a -= 1
        b = idx + 1
        while b < len(line) and (not _is_token_sep(line[b])):
            b += 1
    ...
    def _apply_match_highlight(q20, needle):
        ...
        # ✅ q框优先级最高：q框生效时，不做“跳过当前选区自身”逻辑
        qbox_active = q20._qbox_active()

        tc0 = q20.textCursor()
        sel_start = tc0.selectionStart()
        sel_end = tc0.selectionEnd()
        has_sel = bool(tc0.hasSelection()) and (not qbox_active)
        ...
        pos = 0
        while True:
            c = doc.find(needle, pos)
            if c.isNull():
                break

            if has_sel and c.selectionStart() == sel_start and c.selectionEnd() == sel_end:
                pos = max(pos + 1, c.selectionEnd())
                continue
    ...
    def _update_scroll_marks(q20):
        ...
        if needle:
            L = len(needle)
            if L > 0:
                # ✅ 性能：用 QTextDocument.find；过大时仍避免全量扫
                try:
                    doc_len = int(doc.characterCount())
                except Exception:
                    doc_len = 0

                if doc_len <= 2_000_000 and doc_len > 0:
                    blocks = set()

                    tc = q20.textCursor()
                    sel_start = tc.selectionStart()
                    sel_end = tc.selectionEnd()
                    has_sel = tc.hasSelection() and (not q20._qbox_active())
                    ...
                    while True:
                        ...
                        b = doc.findBlock(c.selectionStart())
                        if b.isValid():
                    ...
                    for bn in sorted(blocks):
                        ...
                        size_factor = max(1.0, font_size / 11.0)
        ...
        if cur.hasSelection():
            ...
            if b1.isValid() and b2.isValid():
                ...
                bb = min(1.0, (b2.blockNumber() / float(total_blocks)) + line_height_ratio)
    ...
    def _scroll_cursor_to_target(q20, from_bottom_line: int = 8):
        try:
            # 先确保光标可见
            q20.ensureCursorVisible()

            # 获取视口高度和滚动条
            viewport_height = q20.viewport().height()
            scroll_bar = q20.verticalScrollBar()

            # 计算目标滚动位置：让光标行位于视口中间偏下
            # 将滚动条值调整为当前值减去视口高度的20%
            # 这样光标行就会显示在视口中间偏下的位置
            target_scroll = scroll_bar.value() - int(viewport_height * 0.2)
    ...
    def jump_to_next_qbox_match(q20, from_bottom_line: int = 8):
        needle = q20._qbox_text()
        if not needle:
            return

        try:
            doc = q20.document()
            start_pos = q20.textCursor().position()

            # 查找下一个匹配项
            c = doc.find(needle, start_pos)
            if c.isNull():
                # 如果没有找到，从文档开头查找
                c = doc.find(needle, 0)
                if c.isNull():
    ...
    def replace_next(q20, needle: str, repl: str):
        if not needle:
            return
        try:
            doc = q20.document()
            ...
            q20.setTextCursor(tc)
            q20.setFocus(Qt.OtherFocusReason)

            q20._schedule_qbox_highlight()
            q20._schedule_debounced_refresh()
        except Exception:
            pass

    def replace_all(q20, needle: str, repl: str):
        if not needle:
            return
        try:
        ...
        if not plain or needle not in plain:
            return

        try:
            cur_pos = q20.textCursor().position()
        except Exception:
            cur_pos = 0
        ...
        try:
            new_plain = plain.replace(needle, repl)
            if new_plain == plain:
                return
            q20.setPlainText(new_plain)

            try:
                tc = q20.textCursor()
                tc.setPosition(min(cur_pos, max(0, len(new_plain))))
                q20.setTextCursor(tc)
            except Exception:
                pass
            ...
            if sv is not None:
                try:
                    q20.verticalScrollBar().setValue(sv)
                except Exception:
                    pass
    ...
    def _on_selection_changed(q20):
        q20._pending_single = False
        try:
            if q20.textCursor().hasSelection():
                q20.q96()
                return
        except Exception:
            pass
    ...
    def q88(q20, event):
        ...
        if hasattr(q20, 'window'):
            window = q20.window()
            if hasattr(window, '_wq_id'):
                bg_color = get_bg_color_by_wq_id(window._wq_id)
...
class q64(QWidget):
    def __init__(q65):
        ...
        q65._is_modifier_pressed = False
        q65._modifier_release_times = []
        q65._MODIFIER_DELAY = 600
    ...
    def _pos_resize_grip(q65):
        ...
        # ✅ 替换条若可见，随窗口变化重新定位
        try:
            if hasattr(q65, "q_replace_panel") and q65.q_replace_panel.isVisible():
                q65._position_replace_panel()
        except Exception:
            pass
    ...
    def _swap_in_real_editor(q65):
        ...
        # ✅ 绑定 q框（最高优先级查找）
        try:
            if hasattr(q65, "q_qbox") and q65.q_qbox is not None:
                q65.q79.bind_qbox(q65.q_qbox)
                try:
                    q65.q_qbox.textChanged.connect(q65.q79._schedule_qbox_highlight)
                except Exception:
                    pass
                try:
                    q65.q_qbox.returnPressed.connect(q65._qbox_enter_jump)
                except Exception:
                    pass
        except Exception:
            pass
        ...
        for w in [q65, q65.q73, q65.q79, q65.q79.viewport(),
                  q65.q_font_box, getattr(q65, "q_qbox", None),
                  getattr(q65, "q_replace_panel", None), getattr(q65, "q_replace_box", None),
                  getattr(q65, "q_btn_rep_one", None), getattr(q65, "q_btn_rep_all", None),
                  q65.q75, q65.q76, q65.q77, q65.q_logo]:
            try:
                if w is not None:
    ...
    def _update_background_color(q65):
        try:
            ...
            bg_color = get_bg_color_by_wq_id(q65._wq_id)

            # ✅ 窗口 / 顶栏统一由函数生成
            q65.setStyleSheet(build_window_qss(bg_color))
            if hasattr(q65, 'q73'):
                q65.q73.setStyleSheet(build_titlebar_qss(bg_color))

            # ✅ 编辑器统一由 apply_editor_theme() 处理（包含：QSS + 选区 palette）
            if hasattr(q65, 'q79'):
                apply_editor_theme(q65.q79, bg_color)
    ...
    def _sync_logo(q65):
        ...
        title = f"{left} : {suffix}"
        q65.q_logo.setText(title)
        try:
            q65.setWindowTitle(title)
        except Exception:
            pass
    ...
    def _position_replace_panel(q65):
        try:
            if not hasattr(q65, "q_replace_panel") or q65.q_replace_panel is None:
                return
            if not hasattr(q65, "q_qbox") or q65.q_qbox is None:
                return
            p = q65.q_qbox.mapTo(q65, QPoint(0, q65.q_qbox.height()))
            q65.q_replace_panel.move(int(p.x()), int(p.y()))
            q65.q_replace_panel.raise_()
        except Exception:
            pass

    def _toggle_replace_panel(q65):
        try:
            if not hasattr(q65, "q_replace_panel") or q65.q_replace_panel is None:
                return
            if q65.q_replace_panel.isVisible():
                q65.q_replace_panel.hide()
            else:
                q65._position_replace_panel()
                q65.q_replace_panel.show()
                q65.q_replace_panel.raise_()
                try:
                    if hasattr(q65, "q_replace_box") and q65.q_replace_box is not None:
                        q65.q_replace_box.setFocus(Qt.OtherFocusReason)
                except Exception:
                    pass
        except Exception:
            pass

    def _qbox_enter_jump(q65):
        try:
            if hasattr(q65, "q79") and hasattr(q65.q79, "jump_to_next_qbox_match"):
                q65.q79.jump_to_next_qbox_match(8)
        except Exception:
            pass

    def _do_replace_one(q65):
        try:
            if not hasattr(q65, "q79"):
                return
            if not hasattr(q65, "q_qbox") or q65.q_qbox is None:
                return
            needle = _normalize_text(q65.q_qbox.text() or "")
            if not needle or needle.isspace():
                return
            repl = ""
            try:
                if hasattr(q65, "q_replace_box") and q65.q_replace_box is not None:
                    repl = _normalize_text(q65.q_replace_box.text() or "")
            except Exception:
                repl = ""
            if hasattr(q65.q79, "replace_next"):
                q65.q79.replace_next(needle, repl)
        except Exception:
            pass

    def _do_replace_all(q65):
        try:
            if not hasattr(q65, "q79"):
                return
            if not hasattr(q65, "q_qbox") or q65.q_qbox is None:
            ...
            try:
                ...
                repl = ""
            if hasattr(q65.q79, "replace_all"):
                q65.q79.replace_all(needle, repl)
        except Exception:
            pass
    ...
    def q68(q65):
        ...
        q65.q73.setStyleSheet(build_titlebar_qss(q1))
        ...
        # ✅ 新增：q框（111px）
        q65.q_qbox = QLineEdit()
        q65.q_qbox.setFixedWidth(111)
        q65.q_qbox.setAlignment(Qt.AlignCenter)
        q65.q_qbox.setPlaceholderText("f")
        q65.q_qbox.setContextMenuPolicy(Qt.NoContextMenu)
        q65.q_qbox.setStyleSheet(
        ...
        q65.q_font_box.setFixedWidth(64)  # ✅ 收窄6px：70 -> 64
        ...
        q65.q_font_box.setContextMenuPolicy(Qt.NoContextMenu)
        ...
        q74.addStretch()
        q74.addWidget(q65.q_qbox)      # ✅ q框在左
        q74.addWidget(q65.q_font_box)  # ✅ 字号框在右
        ...
        # ✅ 替换条：总宽 111px，紧贴在 q框正下方
        q65.q_replace_panel = QWidget(q65)
        q65.q_replace_panel.setVisible(False)
        q65.q_replace_panel.setFixedWidth(111)
        try:
            h = int(q65.q_qbox.sizeHint().height())
        except Exception:
            h = 22
        q65.q_replace_panel.setFixedHeight(h)

        lay = QHBoxLayout(q65.q_replace_panel)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.setSpacing(0)

        q65.q_replace_box = QLineEdit()
        q65.q_replace_box.setFixedHeight(h)
        q65.q_replace_box.setContextMenuPolicy(Qt.NoContextMenu)
        q65.q_replace_box.setStyleSheet(
            "QLineEdit{border:1px solid rgba(0,0,0,40); border-radius:6px; padding:2px; background:rgba(255,255,255,80);}"
            "QLineEdit:focus{border:1px solid rgba(0,0,0,90);}"
        )

        # 按你的“总宽=111px”，按钮极窄：用“替/全”
        q65.q_btn_rep_one = QPushButton("替")
        q65.q_btn_rep_all = QPushButton("全")
        q65.q_btn_rep_one.setToolTip("查找替换")
        q65.q_btn_rep_all.setToolTip("全部替换")
        for b in (q65.q_btn_rep_one, q65.q_btn_rep_all):
            b.setFixedHeight(h)
            b.setFixedWidth(18)
            b.setStyleSheet(
                "QPushButton{border:1px solid rgba(0,0,0,40); border-radius:6px; background:rgba(255,255,255,80); font-size:10px; padding:0px;}"
                "QPushButton:hover{background:rgba(255,255,255,130);}"
            )

        # 剩余给替换框
        q65.q_replace_box.setFixedWidth(max(1, 111 - 18 - 18))

        lay.addWidget(q65.q_replace_box)
        lay.addWidget(q65.q_btn_rep_one)
        lay.addWidget(q65.q_btn_rep_all)

        q65.q_btn_rep_one.clicked.connect(q65._do_replace_one)
        q65.q_btn_rep_all.clicked.connect(q65._do_replace_all)
    ...
    def set_zen(q65, on: bool):
        ...
        # ✅ 进入 zen 时隐藏替换条
        try:
            if on and hasattr(q65, "q_replace_panel") and q65.q_replace_panel is not None:
                q65.q_replace_panel.hide()
        except Exception:
            pass
    ...
    def eventFilter(q65, obj, event):
        # ✅ 规则：右键任何位置 = 只切换 zen（禁用所有右键菜单/右键逻辑）
        ...
        # ✅ 规则：q框获得焦点时，Tab/F2 抬起一次 = 开关替换条（并吃掉 Tab，避免焦点跳走）
        if hasattr(q65, "q_qbox") and obj == q65.q_qbox:
            if event.type() == QEvent.KeyPress:
                try:
                    if event.key() in (Qt.Key_Tab, Qt.Key_F2):
                        event.accept()
                        return True
                except Exception:
                    pass
