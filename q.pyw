...
class q19(QTextEdit):
        ...
        if (e.modifiers() & Qt.ControlModifier) and (e.modifiers() & Qt.ShiftModifier) and e.key() == Qt.Key_V:
            if q20._kope_raw:
                ...
                if not img.isNull():
                    ...
                    # 小于阈值的“插入一张图”本来就是 1 unit，这里不分组也行；为了替换选区也一致，统一用 group
    ...
    def _image_rect_from_pos(q20, img_pos: int, img_size: QSize) -> QRect:
        doc = q20.document()
        if img_pos < 0 or img_pos >= doc.characterCount():
            return QRect()

        c1 = QTextCursor(doc)
        c1.setPosition(img_pos)
        r1 = q20.cursorRect(c1)

        c2 = QTextCursor(doc)
        c2.setPosition(min(img_pos + 1, doc.characterCount() - 1))
        r2 = q20.cursorRect(c2)

        left = min(r1.left(), r2.left())
        right = max(r1.left(), r2.left())
        ...
        if ih <= 0:
            ih = max(r1.height(), r2.height(), 1)

        return QRect(left, top, w, ih)
    ...
    def mouseMoveEvent(q20, e):
        ...
        if (not name) and q20._hover_img_name and q20._hover_img_pos >= 0:
            img_rect = q20._image_rect_from_pos(
                q20._hover_img_pos, q20._hover_img_size)
            if (not img_rect.isNull()) and img_rect.adjusted(-3, -3, 3, 3).contains(e.pos()):
                q20._update_img_bar_pos()
                return
    ...
    def _update_img_bar_pos(q20):
        ...
        if not vp.intersects(img_rect.adjusted(-2, -2, 2, 2)):
            q20._img_bar.hide()
            return
